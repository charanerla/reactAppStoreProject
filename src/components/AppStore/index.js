...
class AppStore extends Component {
  state = {activeTab: tabsList[0], searchValue: ''}

  getActiveTabApps = activeTabId => {
    const resultApps = appsList.filter(
  ...
  changeActiveTabItem = tabId => {
    const resultActiveTab = tabsList.filter(eachTab => eachTab.tabId === tabId)
    this.setState({activeTab: resultActiveTab[0]})
  }
  ...
  render() {
    ...
    const filteredApps = this.getActiveTabApps(activeTab.tabId)

    const searchFilteredApps = filteredApps.filter(eachObj =>
      eachObj.appName.toLowerCase().includes(searchValue),
    )
    ...
    return (
      <div className="app-store-container">
        <div className="app-store-content-container">
          ...
          <ul className="unOrder-list-of-tabItems">
            {tabsList.map(eachTab => (
              <TabItem
                eachTabItem={eachTab}
                key={eachTab.tabId}
                isActive={eachTab.tabId === filteredApps[0].category}
          ...
          <div className="apps-container">
            <ul className="unOrdered-list-of-apps">
              {searchFilteredApps.map(eachApp => (
